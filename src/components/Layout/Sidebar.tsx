    ...
    useEffect(() => {
        ...
        const fetchWeather = async (lat: number, lng: number, cityName?: string) => {
            try {
                ...
                // If coordinate is not default and no city name provided, try to fetch real city name
        ...
        if (savedLocation) {
            ...
            // Default location: Gerung, Lombok Barat
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        // Pass undefined for cityName to trigger fetchCityName
    ...
    const handleManualLocationSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.results && data.results.length > 0) {
                ...
                // Fetch weather for new location, passing name directly to avoid re-fetching via Nominatim
                // We need to re-call the fetchWeather logic. Since it's inside useEffect, we can't call it directly easily.
                // Refactoring fetchWeather outside useEffect or just duplicating specific Logic here.
                // Let's duplicate specific logic for simplicity in this context or create a quick helper if possible.
                // Actually, simplest is to just call the API directly here as we did in fetchWeather.
            ...
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto z-10">
