    ...
    return (
        <header className="h-16 md:h-24 bg-gradient-to-r from-[#002e5d] to-[#001a35] text-white shadow-md flex items-center justify-between px-3 md:px-6 z-20 relative border-b border-white/10">
            <div className="flex items-center gap-4 md:gap-8">
                ...
                {/* Main Navigation - Moved from FloatingMenu */}
                <div className="hidden lg:flex items-center bg-white/5 rounded-full p-1 border border-white/10">
                    {menuItems.map(item => (
                        <button
                            key={item.id}
                            onClick={() => setActivePage(item.id)}
                            className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 ${activePage === item.id ? 'bg-yellow-400 text-slate-900 shadow-sm' : 'text-slate-300 hover:text-white hover:bg-white/10'}`}
                        >
                            {item.label}
                        </button>
                    ))}
